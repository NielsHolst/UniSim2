...
has_iterations = function(M) {
  unique(M$iteration) > 1
}

geom_deduced = function(M) {
  if (max(data.frame(table(M$iteration))$Freq) > 15) geom_point() else geom_line()
}
...
plot_facetted_one_x = function(df, id_x, cols, ytrans, ncol, nrow) {
  M = melted(df, id_x, cols)
...
plot_merged_one_x = function(df, id_x, cols, ytrans, ncol, nrow) {
  M = melted(df, id_x, cols)
  if (ytrans=="log10") M$Value = Log10(M$Value)
  hasIterations = has_iterations(M)
  onlyOneVariable = (length(cols) == 1)
...
plot_facetted_many_x = function(df, id_x, cols, ytrans, ncol, nrow) {
  M = meltxy(df, id_x, cols)
...
plot_merged_many_x = function(df, id_x, cols, ytrans, ncol, nrow) {
  M = meltxy(df, id_x, cols)
  if (ytrans=="log10") M$ResponseValue = Log10(M$ResponseValue)
  hasIterations = has_iterations(M)
  onlyOneVariable = (length(cols) == 1)
...
plot_facetted = function(df, id_x, cols, ytrans, ncol, nrow) {
