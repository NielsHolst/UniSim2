...
sobol_inputs = function(ix_output, nrows=NA) {
  ...
  list(yA=yA(ix_output, n), yB=yB(ix_output, n), yC= yC(ix_output, n))
}
...
sobol_indices = function(y) {
  ...
  rbind(
    # data.frame(
      # Input = input_names,
      # Measure = "FirstOrder0",
      # Effect  = sapply(1:sobol_k, Si)
    # )
    # ,
    # data.frame(
      # Input = input_names,
      # Measure = "Total0",
      # Effect = sapply(1:sobol_k, STi) 
    # )
    # ,
    data.frame(
      Input = input_names,
...
plot_effects = function() {
  ...
  M = adply(1:length(output_names), 1, f)[,-1]
  Msum = ddply(M, .(Output, Measure), g)
  Msum$Input = "Sum"
  M = rbind(M,Msum)
  M

  totals = subset(M, Measure=="Total" & Output==output_names[1])
  totals$Order = order(totals$Effect)
  M = join(M, totals, by=c("Input"))

  ggplot() +
    geom_segment(aes(x=0, xend=Effect, y=Order, yend=Order), subset(M, Measure=="Total"),      colour=colour_total, size=2) +
    geom_segment(aes(x=0, xend=Effect, y=Order, yend=Order), subset(M, Measure=="FirstOrder"), colour=colour_first_order, size=2) +
    geom_vline(xintercept=0, colour="grey") +  
    geom_vline(xintercept=1, colour="grey", size=1) +  
    scale_x_continuous("Effect", breaks=(0:8)/4) +
    scale_y_continuous("", breaks=1:(length(input_names)+1), labels=c(input_names, "Sum")[totals$Order]) +
    facet_wrap(~Output) +
    theme_classic() +
    theme(
