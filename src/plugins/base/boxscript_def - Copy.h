...
namespace boxscript { namespace parser
{
    ...
    auto const bare_date_def = (integer >> '/' >> integer) [dm] |
                        ('/' >> integer >> '/' >> integer) [md];
    auto const bare_date_time_def = bare_date >> lit("T") >> time;
    auto const bool__def = x3::string("TRUE") | x3::string("FALSE");
    ...
    auto const reference_def = path >> port; // >> *(lit_('|') > path > port);
