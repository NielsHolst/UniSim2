...
#include "canopy_photosynthesis_deprecated.h"
...
PUBLISH(CanopyPhotosynthesisDeprecated)

CanopyPhotosynthesisDeprecated::CanopyPhotosynthesisDeprecated(QString name, QObject *parent)
    : Box(name, parent)
{
...
void CanopyPhotosynthesisDeprecated::amend() {
    BoxBuilder builder(this);
    if (!findMaybeOne<Box>("./lightResponse"))
        builder.
        box("vg::LeafLightResponseDeprecated").name("lightResponse").
        endbox();
}

void CanopyPhotosynthesisDeprecated::update() {
    // From Goudriaan & van Laar (1994) but scaled to parAbsorbed,
    // i.e. the total PAR absorbed by the canopy has been computed elsewhere
