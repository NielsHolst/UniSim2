...
#include "leaf_light_response_processes.h"
...
PUBLISH(LeafLightResponseProcesses)

const double O2i = 210;     // [G94, p.80] O2 partial pressure inside stomata [ppm], [mbar], [ml/l]

LeafLightResponseProcesses::LeafLightResponseProcesses(QString name, QObject *parent)
	: Box(name, parent)
{
...
void LeafLightResponseProcesses::reset() {
    update();
}

void LeafLightResponseProcesses::update() {
    // Update basics
    T25 = 25. + T0;
    TleafK = Tleaf + T0;
...
double LeafLightResponseProcesses::maxCarboxylationRate() {
...
double LeafLightResponseProcesses::maxPhotosyntheticCapacity() {
...
double LeafLightResponseProcesses::co2CompensationConcentration() {
...
double LeafLightResponseProcesses::RubiscoCarboxylation() {
