...
VentilationMaxByTemp::VentilationMaxByTemp(QString name, QObject *parent)
    : Box(name, parent)
{
    ...
    Input(dischargeCoefficient).equals(0.7).help("Range [0.6,0.75] (de Jong [1990], p.87)");
    Output(value);
}

void VentilationMaxByTemp::reset() {
    value = 0.;
}

// De Jong (1990), eq. 6.10, p.83
void VentilationMaxByTemp::update() {
