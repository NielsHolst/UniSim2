/* Copyright 2005-2019 by
** Niels Holst, Aarhus University [niels.holst at agro.au.dk] and
** Oliver Koerner, Leibniz-Institute of Vegetable and Ornamental Crops [koerner at igzev.de] and
** Jesper M. Aaslyng, Danish Technological Instutute [jeaa at teknologisk.dk].
** Released under the terms of the GNU Lesser General Public License version 3.0 or later.
** See: www.gnu.org/licenses/lgpl.html
*/
#include <base/phys_math.h>
#include <base/publish.h>
#include "radiation_layer_sky.h"

using namespace base;

namespace vg {

PUBLISH(RadiationLayerSky)

RadiationLayerSky::RadiationLayerSky(QString name, QObject *parent)
    : RadiationLayer(name, parent)
{
    help("holds the radiative parameters of the sky layer");
    Input(propPar).imports("outdoors[propPar]",CA);
    Input(propUv).imports("outdoors[propUv]",CA);
    port("swFluxDown")->imports("outdoors[radiation]",CA);
    port("area")->imports("construction/geometry[groundArea]",CA);
    port("temperature")->imports("outdoors[skyTemperature]",CA);
}

void RadiationLayerSky::initialize() {
    swReflectivityTop = swReflectivityBottom =
    swTransmissivityTop = swTransmissivityBottom =
    lwReflectivityTop = lwReflectivityBottom =
    lwTransmissivityTop = lwTransmissivityBottom = 0.;
    updateAbsorptivities();
}

void RadiationLayerSky::reset() {
    update();
}

void RadiationLayerSky::update() {
    // Anticipate that all UV light will be reflected by the cover (glass); this will increase the proportion of PAR inside
    parFluxDown = 4.57*swFluxDown*propPar/(1. - propUv);
}

} //namespace

