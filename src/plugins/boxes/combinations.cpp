...
Combinations::Combinations(QString name, QObject *parent)
    : Box(name, parent) {
    help("produces all combinations of child values");
    Output(iterations).noReset().help("Total number of combinations");
}
...
void Combinations::amend() {
    ...
    for (Box *child : children)
        (new Port(counterName(child), this))->data(countersPtr++).noReset().access(base::PortAccess::Output);
}
