    ...
    : Box(name, parent)
{
    ...
    Output(date).trackOff().axis("x");
    Output(timeOfDay).trackOff().axis("x");
    Output(trueSolarTime).trackOff().axis("x");
    Output(dateTime).axis("x");
    Output(timeStepSecs).trackOff().axis("x");
    Output(totalTimeSteps).trackOff().axis("x");
    Output(totalTime).trackOff().axis("x");
    Output(totalDays).trackOff().axis("x");
    Output(dayOfYear).trackOff().axis("x");
    Output(day).trackOff().axis("x");
    Output(month).trackOff().axis("x");
    Output(year).trackOff().axis("x");
    Output(hour).trackOff().axis("x");
    Output(minute).trackOff().axis("x");
    Output(second).trackOff().axis("x");
    Output(dateAsReal).trackOff().axis("x");
    Output(dayLength).trackOff();
    Output(sinb).trackOff();
    Output(azimuth).trackOff();
    Output(sunrise).trackOff();
    Output(sunset).trackOff();
    Output(solarConstant).trackOff();
    Output(angot).trackOff();
    Output(irradiationCorrection).trackOff();
}
