...
ValuesBase::ValuesBase(QString name, QObject *parent)
    : Box(name, parent)
{
    Input(counter).help("Current counter into values");
    Output(numValues).noReset().help("Number of values");
    Output(values).noReset().help("Vector of all values");
    Output(value).noReset().help("Current value");
}
