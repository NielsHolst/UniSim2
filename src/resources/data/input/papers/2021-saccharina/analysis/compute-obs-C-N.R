...
write_CN = function(fileName) {
  outFileName = paste0(substr(fileName,1,nchar(fileName)-4), "-CN.txt")
  print(paste("Writing data frame with CN columns added to", outFileName))
...
# Files to process
file_names = c(
  "alge-obs-2019-AVG.txt",
  "alge-obs-2019-AVG-vandrette-liner-100m.txt"
)
file_names = paste0("papers/2021-saccharina/obs/", file_names)

# Plot which you like
plot_CN(file_names[1])
# plot_CN(file_names[2])

# Convert all files
a_ply(file_names, 1, write_CN)
