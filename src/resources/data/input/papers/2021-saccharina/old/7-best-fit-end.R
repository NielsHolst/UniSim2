...
plot_calibration = function(M) {
  ...
  vars = c("dateTime","area","propN","propC")
...
plot_allocation_totals = function(M) {
  obs_trial = unique(M$trial)
  M = melt(M[c("dateTime", "growth", "exudation", "erosion", "respiration")], id.vars="dateTime", value.name="Total", variable.name="Allocation")
...
plot_nitrogen_uptake_rate = function(M) {
  obs_trial = unique(M$trial)
  ggplot(M, aes(dateTime, nitrogenUptakeRate)) +
    geom_hline(yintercept=0) +
    geom_line(colour=green) +
    labs(x="Months", y="Nitrogen uptake rate (g N/g dw/day)\n", title=obs_trial) +
    scale_x_datetime(breaks = date_breaks("months"),
                     labels = date_format("%m")) +
    theme_minimal()
}  
...
print_plots( dlply(sim, .(iteration), plot_nitrogen_uptake_rate) )
print_plots( dlply(sim, .(iteration), plot_calibration) )

# ggplot(sim, aes(growthRatePct)) +
  # geom_histogram()
  
# quantile(sim$growthRatePct, c(0.1,0.25,0.5,0.75,0.9))

sim$Date = date(sim$dateTime)
M = ddply(sim, .(Date), summarise,
  Nwater = mean(1000*N),
  Nuptake = mean(nitrogenUptakeRate*1e6),
  Area = mean(area),
  fArea = mean(fArea),
  sdNitrogen = max(sdN)
)

ggplot(M, aes(Nuptake/Nwater)) +
  geom_histogram()

quantile(M$Nuptake/M$Nwater, c(0.1,0.25,0.5,0.75,0.9))

ggarrange(
  ggplot(M, aes(Nwater, Nuptake)) +
    geom_hline(yintercept=0)+
    geom_point(colour=blue, aes(size=sdNitrogen)) +
    geom_path(colour=blue) +
    geom_abline(intercept=0, slope=c(80,170), colour=red, size=1)+
    theme_classic()
  ,
  ggplot(M, aes(Nwater, Nuptake)) +
    geom_hline(yintercept=0)+
    geom_point(colour=blue, aes(size=Area)) +
    geom_path(colour=blue) +
    geom_abline(intercept=0, slope=c(80,170), colour=red, size=1)+
    theme_classic()
  ,
  ggplot(M, aes(Nwater, Nuptake)) +
    geom_hline(yintercept=0)+
    geom_point(colour=blue, aes(size=fArea)) +
    geom_path(colour=blue) +
    geom_abline(intercept=0, slope=c(80,170), colour=red, size=1)+
    theme_classic()
)
