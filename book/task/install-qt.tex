...
\end{subfigure}%
\caption{Deselect all other components than \gui{Qt 5.7} (or latest version) and \gui{Tools}.}
...
\end{subfigure}%
\caption{Keep only the \gui{MinGW} sub-component of the \gui{Qt 5.7} (or latest version) component. Pick the \gui{64-bit} version if available, otherwise the 32-bit version. There is no need to pick both the 32- and 64-bit versions.}
...
Accept the default suggestions of the installer, \emph{except} when you come to \gui{Select Components}. Which components to install differ a bit between operating systems. For Windows see \iref{fig:qt-setup-win-1} and \iref{fig:qt-setup-win-2}. For Mac OS X see \iref{fig:qt-setup-osx};
...
\end{subfigure}%
\caption{Keep only the \gui{Qt 5.7} (or latest version) and \gui{Tools} components. Inside \gui{Qt 5.7} (or latest version), keep only the \gui{macOS} sub-component.}
...
\item The folder likely missing from this list is called \filename{C:/Qt/5.15.1/mingw81_64/bin} or something similar; the version numbers will keep incrementing with time. 
...
\item Return to to \gui{Environment Variables} and add the folder path you just found to the \gui{Path} variable. First, click on \gui{New} then enter the path, \eg\ \filename{C:/Qt/5.15.1/mingw81_64/bin} (\iref{fig:set-system-path-2}).
\end{enumerate}

The purpose of setting the system path to point to the folder with the Qt libraries is to make it possible to run \US\ by clicking on the executable file \filename{unisim2.exe} found in \filename{\devhome/bin} after you have built from source. However, this might not work; some other program that you installed earlier may also have added a path to its libraries, which may include the Qt libraries but from another Qt version. This chaos is called \emph{DLL hell} (the windows libraries bear the name suffix \filename{.dll}). If you are unlucky, you cannot run \filename{unisim2.exe} directly (you will get a mysterious error message) but only indirectly by running the batch file \filename{run_unisim.bat} found in the \devhome\ folder. Well, it could be worse...

Note, that the DLL hell situation can only occur when you run a \US\ version that you have built yourself. The end-user version is always installed with a copy of all the necessary libraries. Thus it does not need any fiddling with the system path. 
