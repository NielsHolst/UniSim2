...
The resulting transit times can be seen in \cref{tab:vg-geometry} taking into account the variation in the flow rates ($\dot{V}$). Ideally, the two estimates of transit time would be the same, $\Delta t \approx \Delta t_{geom}$. \todo{Her skal vi have m\aa linger fra flere v\ae ksthuse for at se, hvor godt tingene passer sammen og eventuelt udvide modellen, s\aa\ at det passer bedre samme. Indtil videre passer pengene for bottom $23\approx 21.94$, men ikke for top $15\neq 10.73$}

In the steady-state case, when $T_{in}$ and $T_{air}$ remain constant, so will $T_{out}$ \eqref{eq:vg-regression}. We can then calculate the effect per greenhouse area($P$; \si{\watt\square\meter}) of the pipe as
\begin{equation}
